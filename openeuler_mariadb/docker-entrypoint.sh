...
mysql_log() {
	local type="$1"; shift
	local output=""; 
	output=$(printf '%s [%-0s] [Entrypoint]: %s' "$(date '+%Y-%m-%d %H:%M:%S')" "$type" "$*")
	echo -e "$output"
}
mysql_note() {
	mysql_log Note "$@"
...
docker_init_database_dir() {
	...
	if { /usr/local/mysql/scripts/mysql_install_db --help || :; } | grep -q -- '--skip-test-db'; then
		# 10.3+
		installArgs+=( --skip-test-db )
	fi
	# "Other options are passed to mariadbd." (so we pass all "mysqld" arguments directly here)
	/usr/local/mysql/scripts/mysql_install_db "${installArgs[@]}" "${@:2}" --default-time-zone=SYSTEM --enforce-storage-engine= --skip-log-bin
...
docker_setup_db() {
	...
	if [ -n "$MARIADB_RANDOM_ROOT_PASSWORD" ]; then
		...
		mysql_note "${REDB}GENERATED ROOT PASSWORD: $MARIADB_ROOT_PASSWORD${NC}"
	fi
	...
	mysql_note "${REDB}Securing system users (equivalent to running mysql_secure_installation)${NC}"
